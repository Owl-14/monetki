...
test("проведение проверяет права, сохраняет bankId и защищено от дубля", async () => {
  const [actions, http, migration] = await Promise.all([
    source("actions.ts"),
    source("http.ts"),
    readFile(new URL("../supabase/migrations/004_process_bank_transaction.sql", import.meta.url), "utf8"),
  ]);
...
test("атомарная функция применяется всеми банковскими workflow", async () => {
  ...
  for (const path of paths) {
    ...
    assert.match(workflow, /supabase\/migrations\/004_process_bank_transaction\.sql/, path);
  }
});
