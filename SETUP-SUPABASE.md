...
## Шаг 7. Обязательно включить ежечасную синхронизацию с банком

Деплой Edge Function **не применяет миграции**. Поэтому без этого шага функция будет
работать, но автоматическая синхронизация с Точка Банком не запустится.

1. В Supabase откройте **SQL Editor** → **New query**.
2. Вставьте целиком содержимое файла
   [`supabase/migrations/002_schedule_tochka_sync.sql`](supabase/migrations/002_schedule_tochka_sync.sql).
3. Нажмите **Run**. Миграцию можно запускать повторно: она удалит все старые задания
   `tochka-sync-hourly` и создаст ровно одно задание на пятой минуте каждого часа с
   таймаутом 180 секунд.
...
`https://pmntdxwdsrdtaindabqb.supabase.co/functions/v1/api`, действие `tochka_sync` и
...
Ручной ответ `tochka_sync` содержит только безопасные агрегаты: количество счетов,
