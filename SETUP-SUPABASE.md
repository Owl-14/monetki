...
4. Workflow обновит Edge Function Secret, создаст или обновит запись Vault, идемпотентно
   пересоздаст ровно одно задание `tochka-sync-hourly` на пятой минуте каждого часа и сразу
   выполнит защищённую проверочную синхронизацию за последние 30 дней.
