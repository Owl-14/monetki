...
1. В Supabase: **Edge Functions** (меню слева) → вкладка **Secrets** → **Add new secret**:
   ...
   - прежний `TOCHKA_UNIT` больше не нужен: новые операции попадают в «Необработанные», а бизнес назначается при проведении
   - `BANK_SIGNAL_HMAC_SECRET` можно задать отдельной случайной строкой длиной от 32 символов для стабильных псевдонимов счетов; если он не задан, используется уже случайный `TOCHKA_SYNC_SECRET`. Значение никогда не выводится и не хранится в репозитории
2. Сохранить.
...
операций и ошибок. Так как у Get Statement нет пагинации, ровно 100 операций считаются
признаком возможного усечения ответа, а не документированной страницей: многодневный
диапазон рекурсивно делится на непересекающиеся половины по календарным дням. За один
запуск допускается не больше 32 выписок, глубина не больше 8 и общий срок 150 секунд.
Поля `split`, `truncated`, `inconsistent`, `requestLimitReached`, `depthLimitReached` и `deadlineReached`
показывают только безопасные счётчики без дат,
идентификаторов и банковских данных. Если один календарный день всё ещё содержит 100
операций либо достигнут защитный лимит, ответ обязательно получает `outcome: partial`,
а `LAST_SYNC` не обновляется как полностью успешный.

`outcome` различает `zero_transactions`, `all_duplicates`, `partial`
...
Параллельные ручные и cron-запуски защищены серверной арендой
`records('_locks', 'tochka_sync')`. Атомарные RPC из миграции
`003_tochka_sync_lease.sql` разрешают только один запуск, автоматически перехватывают
просроченную аренду через 210 секунд и освобождают только аренду с собственным
`leaseId`. Конкурирующий вызов получает безопасный `outcome: busy` и не меняет отметки
активной синхронизации. Миграция применяется автоматически перед деплоем Edge Function.

Миграция `006_recover_hidden_bank_transactions.sql` исправляет исторический разрыв после
переезда 20 июля: банковские строки `finance`, записанные без существующего бизнеса,
атомарно возвращаются в «Необработанные». Строки действующих и архивных бизнесов не
затрагиваются, а повторный deploy не создаёт дублей. В интерфейсе администратор видит
только безопасные счётчики и границы дат восстановления. Строки без исходного
`bankId` не перемещаются автоматически и выводятся отдельным безопасным счётчиком;
ссылка складского движения на восстанавливаемую строку останавливает всю миграцию.

Миграция `009_bank_rules.sql` добавляет безопасные правила банковской очереди. Она:

- проверяет отсутствие прежних дублей `finance.bankId`/`bankTransactions.bankId` и останавливается, если их нужно сначала разобрать вручную;
- создаёт уникальные индексы, неизменяемую историю версий правил и настроек (global auto = off), append-only аудит и атомарные RPC enqueue/apply/correct/reverse/restore;
- применяется **до** публикации Edge Function workflow `deploy-backend.yml`; те же банковские workflow явно включают файл 009;
- ничего не включает в production автоматически: после merge администратор сначала открывает **Финансы → Необработанные → Настройки**, запускает **Проверить на очереди**, задаёт малые лимиты и только затем вручную подтверждает включение auto.
