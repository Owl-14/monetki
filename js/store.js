...
// Копия логики из supabase/functions/api/rules.js — менять оба места.
...
export class LocalStore {
  ...
  async create(token, entity, item) {
    ...
    if (entity === 'bankAutoRules') {
      item = normalizeBankAutoRule(item);
  ...
  async applyBankRules(token) {
    const u = this._user(token);
    if (!u || u.role !== 'admin') return { ok: false, error: 'Только для админа' };
    let applied = 0;
    let failed = 0;
    for (const transaction of [...this._db().bankTransactions]) {
      if (transaction.ignored) continue;
      const db = this._db();
      const rule = matchBankAutoRule(transaction, db.bankAutoRules, db.businesses);
      if (!rule) continue;
      const result = await this.processBankTransaction(token, transaction.id, rule.businessId, rule.category);
      if (!result.ok) { failed++; continue; }
      if (!result.alreadyProcessed) {
        const after = this._db();
        const finance = after.finance.find((entry) => entry.id === result.item.id);
        if (finance) {
          if (rule.owner) finance.owner = rule.owner;
          finance.ruleId = rule.id;
          this._save(after);
        }
      }
      applied++;
  ...
  async ignoreBankTransaction(token, id, ignored = true) {
    const u = this._user(token);
    if (!u || u.role !== 'admin') return { ok: false, error: 'Только для админа' };
    const db = this._db();
    const transaction = db.bankTransactions.find((entry) => entry.id === id);
    if (!transaction) return { ok: false, error: 'Банковская операция не найдена' };
    transaction.ignored = ignored !== false;
...
class RemoteStore {
  ...
  applyBankRules(token) { return this._call({ action: 'apply_bank_rules', token }); }
  ignoreBankTransaction(token, id, ignored = true) { return this._call({ action: 'ignore_bank_transaction', token, id, ignored }); }
