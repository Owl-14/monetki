// Сертификаты НУЦ Минцифры. С августа 2026 года API Точки (enter.tochka.com) отдаёт
// сертификат, выпущенный «Russian Trusted Sub CA», которого нет в стандартном
// списке доверенных корней Deno — из-за этого выписки перестали приходить.
// Доверие к ним подключается ТОЛЬКО для HTTP-клиента запросов к Точке (bank.ts).
// Источник: https://gu-st.ru/content/Other/doc/ (Госуслуги).
// Root CA  SHA-256: D26D2D0231B7C39F92CC738512BA54103519E4405D68B5BD703E9788CA8ECF31 (до 2032-02-28)
// Sub CA   SHA-256: 2155785036C900DBB5F1BB2A1569C80C55595BD6BF94867A29BBDDBC7D88A3F2 (до 2029-07-19)

export const RUSSIAN_TRUSTED_ROOT_CA = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;

export const RUSSIAN_TRUSTED_SUB_CA = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
