...
async function insertRow(entity: string, item: Rec) {
...
async function acquireTochkaSyncLease() {
  ...
  const { data, error } = await db.rpc("acquire_tochka_sync_lease", {
    p_lease_id: leaseId,
...
async function releaseTochkaSyncLease(leaseId: string) {
  const { error } = await db.rpc("release_tochka_sync_lease", { p_lease_id: leaseId });
...
async function runTochkaSync(days = 30) {
  ...
  try {
    ...
    try {
      ...
      const complete = result.ok && result.outcome !== "partial";
      await kvSet(
        "LAST_SYNC_ERROR",
