...
`apply_bank_rules` (только администратор; разносит очередь по `bankAutoRules`), `ignore_bank_transaction{id,ignored}` (только администратор; скрыть/вернуть операцию очереди),
...
- Счёт учредителя = `businessOwners.opening` + операции «Расчеты с учредителями» с его `responsible` + его доля прибыли всех турниров + его часть «Общих хозяйственных». Общий расход относится к ближайшему неотменённому турниру в этот день или позже (иначе — к последнему прошедшему) и делится по его пресету: ДШ — целиком dmitry, ОБЩ — 33/34/33 (`generalExpenseShares`). `generalShares` (2/3·1/6·1/6) — только запасной вариант, когда турниров с пометкой нет.
...
- Проведённая банковская операция — обычная запись `finance`: её можно редактировать и переносить между бизнесами. Удаление вернёт её в очередь при следующей выгрузке (дедуп по bankId).
- Автоправила: после каждой синхронизации (и по кнопке) `applyBankAutoRules()` проводит подходящие операции очереди через тот же `process_bank_transaction`, затем дописывает `owner`/`ruleId`. Ошибка одного правила или одной операции не останавливает остальные.
- Синхронизация не должна падать целиком из-за одной операции: ошибка сохранения считается (`saveFailed`) и делает результат частичным. В `LAST_SYNC_ERROR` пишется только безопасная причина (код HTTP банка, таймаут), без тела ответа.
