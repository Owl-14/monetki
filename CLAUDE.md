...
- Экономика событий: начислено = активные регистрации; оплачено/депозит/возврат/прямой расход считаются только по неизменяемым `eventFinanceAllocations` к реальным `finance`; прибыль = фактический доход − прямые расходы − возвраты; маржа и прибыль на участника вычисляются из того же снимка. Доли типа должны давать 100%; последняя доля получает копеечный остаток, поэтому распределение точно равно прибыли. `007_atomic_event_finance.sql` содержит RPC, блокировки и защитный trigger.
