...
`bank_rule_journal{limit?,cursor?}`, `bank_rule_correct`, `bank_rule_reverse` (специальный admin-only жизненный цикл; журнал использует валидируемый keyset-cursor `(created,id)`, обычный CRUD системных сущностей запрещён),
